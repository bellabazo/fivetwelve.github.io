...
class Board(GameElement):
        ...
        for row in range(len(values)): 
            ...
            for col in range(len(values[row])):
                ...
                if val == 0:
                    ...
                    pos = Vec(row, col) # help from Youtube
                    row_tiles.append(Tile(pos, val))
            self.tiles.append(row_tiles)

    def in_bounds(self, pos: Vec) -> bool:
        """Is position (pos.x, pos.y) a legal position on the board?"""
        return 0 <= pos.x < len(self.tiles) and 0 <= pos.y < len(self.tiles[0]) # self.tiles[0] from ai
    ...
    def left(self):
        for row in range(len(self.tiles)):
            for col in range(1, len(self.tiles[0])):  
                self.slide(Vec(row, col), self.LEFT)
    
    def right(self):
        for row in range(len(self.tiles)):
            for col in range(len(self.tiles[0])-2, -1, -1):
                self.slide(Vec(row, col), self.RIGHT)

    def up(self):
        for col in range(len(self.tiles[0])):
            for row in range(1, len(self.tiles)):
                self.slide(Vec(row, col), self.UP)
    
    def down(self):
        for col in range(len(self.tiles[0])):
            for row in range(len(self.tiles)-2, -1, -1):
                self.slide(Vec(row, col), self.DOWN)
